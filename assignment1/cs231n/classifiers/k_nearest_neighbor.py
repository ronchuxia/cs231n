...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i, j] = np.sqrt(np.sum(np.power(test_sample - train_sample, 2)))
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            test_sample = X[i]
            dists[i] = np.sqrt(np.sum(np.power(test_sample - self.X_train, 2), axis=1))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            k_nearest_neighbors = np.argsort(dists[i])[:k]  # indices of the k nearest neighbors in the training set
            closest_y = self.y_train[k_nearest_neighbors]   # labels of the k nearest neighbors           
            ...
            label_occurences = np.bincount(closest_y)   # count the occurrences of the labels
            y_pred[i] = np.argmax(label_occurences) # find the first label with the maximum occurrences 
